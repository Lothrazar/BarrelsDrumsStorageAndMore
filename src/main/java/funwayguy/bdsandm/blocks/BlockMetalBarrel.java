...
public class BlockMetalBarrel extends BlockBarrelBase implements IBdsmColorBlock {
  ...
  @Override
  public int getColorCount(World blockAccess, BlockState state, BlockPos pos) {
  ...
  @Override
  public int[] getColors(World blockAccess, BlockState state, BlockPos pos) {
    TileEntity tile = blockAccess.getTileEntity(pos);
    if (tile == null || !tile.hasCapability(BdsmCapabilies.BARREL_CAP, null)) {
      return new int[0];
    }
  ...
  @Override
  public void setColors(World blockAccess, BlockState state, BlockPos pos, int[] color) {
    TileEntity tile = blockAccess.getTileEntity(pos);
    if (tile == null || !tile.hasCapability(BdsmCapabilies.BARREL_CAP, null)) {
      return;
    }
